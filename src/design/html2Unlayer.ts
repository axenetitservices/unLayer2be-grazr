...
export class Html2Unlayer {
  ...
  getDesign() {
    ...
    design.body = {
      rows: parseRows(this.body).map((row: any, i) => {
        ...
        const hasMultipleCells = this.hasMultipleCell(
          Array.from(row.children[0]?.children ?? [])
        );
        const columnsBackgroundColor = this.getColor(
          row?.children[0]?.children[0]?.style["background-color"]
        );
  ...
  getCells = (columns: any[]) => {
    const cells = columns.map((x) =>
      Number(x.values.size["width"]?.replace(/[a-zA-Z]+/g, ""))
    );
  ...
  hasMultipleCell = (columns: any[]) => {
    const cells = columns?.map((x) =>
      Array.from(parseChildren(x.children))?.map((y: any) =>
        y.classList.contains("unlayer2be")
          ? -1
          : Number(
              (y?.style["min-width"] ?? y?.style["width"])?.replace(
                /[a-zA-Z]+/g,
                ""
              )
            )
  ...
  getColumns = (columns: any[]) =>
    Array.from(columns).map((column: any, i) => {
      this.countElement("column");
      const style = column?.style;
      const padding = Array.from(parseChildren(column.children, true))[0]
  ...
  calculateColumnsRatio = (cells: any[]) => {
    return cells?.includes(-1)
      ...
      ? cells?.map((x: any) =>
          !isNaN(x) && x
            ? Math.round(
                x / Math.min(...cells.map((y: any) => (y == 0 ? 1 : y)))
              )
            : 12
        )
      : [12];
  ...
  getContents = (column: any) =>
    Array.from(parseChildren(column.children, true)).map((content: any, i) => {
  ...
  getImage = (content: Element) =>
    Object.assign(
      {},
      {
        src: {
          url: content?.querySelector("img")?.src,
          width: content?.querySelector("img")?.width
            ? content?.querySelector("img")?.width
            : "auto",
          height: content?.querySelector("img")?.height
            ? content?.querySelector("img")?.height
            : "auto",
        },
      }
    );
   ...
   */
  getStyle = (style: any, text: any = "", id_type?: string) =>
    style
      ? Object.assign(
          {},
          {
